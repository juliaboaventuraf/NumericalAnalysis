...
#Questão 4- a)
expressao_questao_4a = relative_error(
    Float64(factorial(7)),
    sqrt(2 * Float64(pi) * 7) * (7 / Float64(exp(1)))^7,
)
resposta_questao_4a = 0.011826223886416323

#Questão 4- b)
expressao_questao_4b = relative_error(
    Float64(factorial(20)),
    sqrt(2 * Float64(pi) * 20) * (20 / Float64(exp(1)))^20,
)
...
trunc_x = trunc(
    trunc_sum(13 / 14, -6 / 7, digits = 3) / trunc_sum(2 * exp(1), -5.4, digits = 3),
    digits = 3,
)
valor_trunc_x = 2.028
...
round_x = round(
    round_sum(13 / 14, -6 / 7, digits = 3) / round_sum(2 * exp(1), -5.4, digits = 3),
    digits = 3,
)
valor_round_x = 1.946
